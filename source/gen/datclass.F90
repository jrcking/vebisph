program datgen
  ...
  write(*,*) '  case  4:  Taylor-Couette flow'
  write(*,*) '  case  5:  Grilli flow'
  write(*,*) '  case  6:  Kolmogorov flow'
...
!! ------------------------------------------------------------------------------------------------
  case(4)
     ...
     !! Domain size:
     xl=4.0d0;yl=4.0d0   !! Should be of order unity, as simulation is dimensionless
     h0=0.5d0
     ...
     body_force = (/0.0d0,0.0d0/)    
     ve_nonlinearity=100d0;alpha_evss=0.0d0
     beta = 1.0d0;Ra=1.0d2;Pr=1.0d0;El=0.0d0

     !! Resolution:
     dx=h0/30.0;h=1.3d0*dx      !! N.B. you should only need to change dx

     !! Build domain
     nb_patches = 4
     allocate(b_node(nb_patches,2),b_edge(nb_patches,2))
     allocate(b_type(nb_patches),b_periodic_parent(nb_patches))
     b_type(:) = (/ 0,0,0,0 /)                                    !! Need to construct a box of non-existent boundaries
     b_periodic_parent(:) = (/ 3, 4, 1, 2 /)                      !! to contain the two cylinders
     b_node(1,:) = (/ -0.5d0*xl, -0.5d0*yl /)
     b_node(2,:) = (/ 0.5d0*xl, -0.5d0*yl /)
     b_node(3,:) = (/ 0.5d0*xl, 0.5d0*yl /)
     b_node(4,:) = (/ -0.5d0*xl, 0.5d0*yl /)
     nb_circles = 2
     allocate(c_centre(nb_circles,2),c_radius(nb_circles),c_omega(nb_circles),c_vel(nb_circles,2))
     c_centre(1,:) = (/ 0.0d0,0.0d0*h0 /);c_radius(1)=h0;c_omega(1)=1.0d0;c_vel(1,:)=0.0d0
     c_centre(2,:) = (/ 0.0d0,0.0d0 /);c_radius(2)=-2.0d0*h0;c_omega(2)=-1.0d0;c_vel(2,:)=0.0d0
     call make_boundary_edge_vectors
     xb_min = minval(b_node(:,1));xb_max = maxval(b_node(:,1));yb_min = minval(b_node(:,2));yb_max = maxval(b_node(:,2))
     ...
     ! -- fluid particles-- 
     x = xb_max - 0.5*dx
     do while(x.ge.xb_min)
        y = yb_max - 0.5*dx
        do while(y.ge.yb_min)
           if((x-c_centre(1,1))**2+(y-c_centre(1,2))**2.gt.(c_radius(1)+0.25*dx)**2) then  !! Don't fill the inner 
           if((x-c_centre(2,1))**2+(y-c_centre(2,2))**2.lt.(-c_radius(2)-0.25*dx)**2) then  !! only within the outer
                 ipart=ipart+1;xp(ipart)=x;yp(ipart)=y;p(ipart) =0.0d0;up(ipart)=0.0d0;vp(ipart)=0.0d0
           end if
           end if
           y = y - dx
        enddo
...
!! ------------------------------------------------------------------------------------------------
  case(5)
     ...
     !! Physical parameters
     body_force = (/30.0d0,0.0d0/)    
     ve_nonlinearity=100d0;alpha_evss=0.0d0
     beta = 0.1d0;Ra=1.0d-2;Pr=1.0d0;El=1.0d0
...
!! ------------------------------------------------------------------------------------------------
  case(6)
     ...
     !! Kolmogorov flow: periodic box size length 2pi
     
     !! Domain size:
     xl=2.0d0*pi;yl=xl   !! Should be of order unity, as simulation is dimensionless
     h0=0.5d0
...
!! ------------------------------------------------------------------------------------------------
  ...
  open(13,file='./IPART')
  do i=1,np
     write(13,*) xp(i), yp(i), up(i), vp(i), 0.0d0, dx*dx
  end do
  close(13)
  deallocate(xp, yp,up,vp,p,vol)
  write(*,*) 'np = ',np,"nbS = ",nbS
  print*,'dx',dx,'h',h
...
!! ------------------------------------------------------------------------------------------------
   subroutine make_boundary_particles
     ...
     ipart=0
     allocate(xp(npar), yp(npar),up(npar), vp(npar),p(npar), vol(npar))
